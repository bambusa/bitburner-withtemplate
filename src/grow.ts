...
export async function main(ns : NS) : Promise<void> {
    ...
    if (server.moneyAvailable == server.moneyMax || server.hackDifficulty != server.minDifficulty) {
        console.log("#predictionError growing at server.moneyAvailable "+server.moneyAvailable+" == server.moneyMax "+server.moneyMax+" || server.hackDifficulty "+server.hackDifficulty+" != server.minDifficulty "+server.minDifficulty);
    }
